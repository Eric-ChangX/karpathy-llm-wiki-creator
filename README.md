...
- `raw/` holds source material, organized into typed subdirs (`articles/`, `clippings/`, `images/`, `pdfs/`, `notes/`, `personal/`); **immutable** after ingest
...
- `scripts/lint.py` runs 9 mechanical health checks (broken links, orphans, frontmatter validity, type/dir consistency, source-raw integrity, outputs ref integrity, index completeness, log format)
...
└── templates/                     # Files & directories copied verbatim into each new KB
    ...
    ├── README.md                  # Human-facing intro (with {{DOMAIN}} placeholder)
